<template>
	<view class="padding_top3">
			...
			<view>
				<view class="page_widthMoudel">
					<view v-for="(item, index) in graphicData" :key="index" class="padding_top3 padding_bottom3 border_bottom">
						<view class="uni-flex">
							<view class="width15 margin_top2 text_center">
								<image style="width: 62upx;height: 62upx;border-radius: 6upx;" :src="item.logo" mode=""></image>
							</view>
							<view class="width55">
				...
				<view class="">
					<swiper class="imageContainer" circular autoplay>
						<block v-for="(item, index) in promotionPosterData" :key="index">
							<swiper-item @click="swiperClick(item)"><image class="itemImg" :src="item.poster" lazy-load mode="scaleToFill"></image></swiper-item>
						</block>
					</swiper>
				</view>
		...
		<template v-if="tabIndexT == 2">
			<view class="page_widthMoudel">
				<view class="">
					<swiper class="imageContainer" circular autoplay style="height: 70vh;">
						<block v-for="(item, index) in promotionPosterData" :key="index">
							<swiper-item @click="swiperClick(item)"><image style="height: 70vh;" class="itemImg" :src="item.poster" lazy-load mode="scaleToFill"></image></swiper-item>
						</block>
					</swiper>
				</view>
				<view class="uni-flex padding_bottom3 " style="position: fixed;bottom: 0;width: 92%;">
...
export default {
	data() {
		return {
			...
			tabIndexOne: 0,
			shareList:[
				{name:'分享海报',img:'../../static/image/icon/fxh.png'},
				{name:'保存海报',img:'../../static/image/icon/bcx.png'}
			],
			tabListTwo:[
