					 ...
					 <view class="uni-swiper-msg margin_left2">
						...
						<swiper class="imageContainer" circular autoplay>
							<block v-for="(item, index) in posterData" :key="index">
								<swiper-item @click="swiperClick(item)"><image class="itemImg" :src="item.posterUrl" lazy-load mode="scaleToFill"></image></swiper-item>
							</block>
						</swiper>
					 </view>
					...
					<view v-for="(item, index) in rebateData" :key="index" class="padding_top3 padding_bottom3 border_bottom">
						<view class="uni-flex">
							<view class="width15 margin_top2 text_center">
								<image style="width: 62upx;height: 62upx;border-radius: 6upx;" :src="item.picture" mode=""></image>
							</view>
							<view class="width55">
...
export default {
	data() {
		return {
			...
			imgList:[
				{url:'../../static/image/icon/1.png'},
				{url:'../../static/image/icon/2.png'},
				{url:'../../static/image/icon/3.png'},
				{url:'../../static/image/icon/4.png'}
			],
			tabIndexT: 0,
			shareFalg: false,
			posterData:[],
